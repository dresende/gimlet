var Record   = require("../Record").Record;
var Property = require("../Property").Property;

exports.create = function (pkg, uri, settings) {
	return new Driver(require(pkg).createPool(uri), settings || {});
};

function Driver(pool, settings) {
	this.pool       = pool;
	this.settings   = settings;

	this.__releaseTimeout = this.settings.releaseTimeout * 1000;;
}

Driver.prototype.connection = function () {
	var driver     = this;
	var connection = null;
	var $          = {
		query: function () {
			if (driver.extensions.prepare($)) {
				driver.extensions.emit("connection", driver.extensions.connection);
			}

			var args = Array.prototype.slice.apply(arguments);
			var next = args[args.length - 1];
			var con  = null;

			if (typeof next == "function") {
				args[args.length - 1] = (err, rows, info) => {
					if (con) con.release();

					if (err) {
						return (typeof next == "function" ? next(err) : null);
					}

					var props = {}, alias = {};

					if (info) {
						for (var i = 0; i < info.length; i++) {
							props[info[i].name] = define_property(alias, info[i]);
						}
					} else if (rows && !Array.isArray(rows)) {
						info = rows;
						rows = null;
					}

					return next(null, rows ? rows.map((row) => {
						for (var k in alias) {
							row[alias[k]] = row[k];
							delete row[k];
						}
						return new Record(this, row, props);
					}) : info);
				};
			}

			if (connection !== null) {
				connection.query.apply(connection, args);
				return this;
			}

			driver.getConnection((err, _con) => {
				if (err) {
					return (typeof next == "function" ? next(err) : null);
				}

				if (typeof next != "function") {
					args.push(() => {
						con.release();
					});
				}

				con = _con;

				con.query.apply(con, args);
			});

			return this;
		},
		queryRow: function () {
			var args = Array.prototype.slice.apply(arguments);

			if (typeof args[args.length - 1] != "function") {
				return this.query.apply(this, args);
			}

			var next = args.pop();

			args.push(function (err, rows) {
				if (err) return next(err);

				return next(null, rows[0] || null);
			});

			return this.query.apply(this, args);
		},
		queryOne: function () {
			var args = Array.prototype.slice.apply(arguments);

			if (typeof args[args.length - 1] != "function") {
				return this.query.apply(this, args);
			}

			var next = args.pop();

			args.push(function (err, rows) {
				if (err) return next(err);
				if (rows.length === 0) return next(null, null);

				var k = Object.keys(rows[0]);

				return next(null, k.length ? rows[0][k[0]] : null);
			});

			return this.query.apply(this, args);
		},
		create: function (table, data, next) {
			return this.query({
				sql    : "INSERT INTO ?? SET ?",
				values : [ table, data ]
			}, next);
		},
		save: function (table, changes, conditions, next) {
			var query = {
				sql    : "UPDATE ?? SET ",
				values : [ table ]
			};
			var tmp     = build_conditions(changes, ", ");

			query.sql   += tmp.sql;
			query.values = query.values.concat(tmp.values);

			tmp = build_conditions(conditions);

			query.sql   += " WHERE " + tmp.sql;
			query.values = query.values.concat(tmp.values);

			return this.query(query, next);
		},
		remove: function (table, conditions, next) {
			var query = {
				sql    : "DELETE FROM ??",
				values : [ table ]
			};
			var tmp     = build_conditions(conditions);

			if (tmp.sql) {
				query.sql   += " WHERE " + tmp.sql;
				query.values = query.values.concat(tmp.values);
			}

			return this.query(query, next);
		},
		transaction: function (next) {
			if (connection) {
				return (next ? next(new Error("Transaction already in course")) : null);
			}

			driver.getConnection((err, con ) => {
				if (err) {
					return (next ? next(err) : null);
				}

				connection = con;

				connection.beginTransaction((err) => {
					if (err) {
						connection.release();
						connection = null;

						return (next ? next(err) : null);
					}

					return (next ? next() : null);
				});
			});
		},
		commit: function (next) {
			if (!connection) {
				return (next ? next(new Error("No transaction connection found")) : null);
			}

			connection.commit((err) => {
				connection.release();
				connection = null;

				return (next ? next(err) : null);
			});
		},
		rollback: function (next) {
			if (!connection) {
				return (next ? next(new Error("No transaction connection found")) : null);
			}

			connection.rollback((err) => {
				connection.release();
				connection = null;

				return (next ? next(err) : null);
			});
		},
		close: function () {
			driver.pool.end.apply(driver.pool, arguments);
		}
	};

	$.extensions = this.extensions;

	return $;
};

Driver.prototype.getConnection = function (next) {
	if (!this.settings.debug || !this.__releaseTimeout) {
		return this.pool.getConnection(next);
	}

	this.pool.getConnection((err, con) => {
		if (err) return next(err);

		var release = con.release.bind(con);
		var timer   = setTimeout(() => {
			process.stderr.write("(GIMLET) " + (new Date) + " [!] " +
			                     "Connection to database held from pool for too long " +
			                     "(more than " + this.settings.releaseTimeout + " seconds)\n");

			delete timer;
		}, this.__releaseTimeout);

		con.release = function () {
			if (timer) {
				clearTimeout(timer);
				delete timer;
			}

			return release();
		};

		return next(null, con);
	});
};

Driver.prototype.ping = function () {
	this.pool.ping.apply(this.pool, arguments);
};

Driver.prototype.open = function (next) {
	this.getConnection((err, con) => {
		if (err) {
			return (next ? next(err) : null);
		}

		con.release();

		return (next ? next() : null);
	});
};

Driver.prototype.close = function () {
	this.pool.end.apply(this.pool, arguments);
};

function build_conditions(conditions, join) {
	var sql = [], values = [];

	for (var k in conditions) {
		sql.push("?? = ?");
		values.push(k);
		values.push(conditions[k]);
	}

	return {
		sql    : sql.join(join || " AND "),
		values : values
	};
}

function define_property(alias, opts) {
	if (!opts.orgTable) {
		var new_name = opts.name.toLowerCase().replace(/[^a-z0-9]/g, "_").replace(/_+$/, "");

		if (new_name != opts.name) {
			alias[opts.name] = new_name;
		}

		return new Property(opts.name, {
			table        : null,
			primary      : false,
			type         : convert_type(opts.columnType),
			len          : opts.length
		});
	}

	return new Property(opts.orgName, {
		table        : opts.orgTable,
		primary      : !!(opts.flags & 0x2),
		type         : convert_type(opts.columnType),
		len          : opts.length,
		defaultValue : opts["default"],
		notNull      : !!(opts.flags & 0x1)
	});
}

function convert_type(n) {
	switch (n) {
		case   0: // DECIMAL
		case   1: // TINYINT
		case   2: // SMALLINT
		case   3: // INT
		case   4: // FLOAT
		case   5: // DOUBLE
		case   8: // BIGINT
		case   9: // MEDIUMINT
		case 246: // NEWDECIMAL
			return "number";
		case   7: // TIMESTAMP
		case  10: // DATE
		case  11: // TIME
		case  12: // DATETIME
		case  13: // YEAR
		case  14: // NEWDATE
			return "date";
		case  15: // VARCHAR
		case 249: // TINYBLOB, TINYTEXT
		case 250: // MEDIUMBLOB, MEDIUMTEXT
		case 251: // LONGBLOB, LONGTEXT
		case 252: // BLOB, TEXT
		case 253: // VARCHAR, VARBINARY
		case 254: // CHAR, BINARY
			return "text";
		case  16: // BIT
			return "boolean";
		case 247: // ENUM
		case 248: // SET
			return "enum";
		case 255: // GEOMETRY
			return "geom";
	}
}

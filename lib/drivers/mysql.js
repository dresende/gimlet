...
SqlString.objectToValues = (obj, tz) => {
	...
	for (let key in obj) {
		...
		if (val && typeof val == "object" && Object.keys(val).length == 2 && typeof val.x == "number" && typeof val.y == "number") {
...
class Driver {
	...
	connection() {
		...
		let $          = {
			...
			commit: (next = () => {}) => {
				if (!connection) return next(new Error("No transaction connection found"));

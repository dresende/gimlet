		...
		Object.defineProperty(e.record, "remove", {
			value: function (next) {
				...
				var do_next = function (n) {
					...
					e.connection.remove(table[n], updates[table[n]].id, function (err) {
						if (err) {
							if (next) {
								next(err);
							} else {
		...
		Object.defineProperty(e.record, "save", {
			value: function () {
				...
				for (var k in changes) {
					e.data[k] = e.props[k].validate(changes[k]);
				}
				...
				var do_next = function (n) {
					...
					e.connection.save(table[n], updates[table[n]].data, updates[table[n]].id, function (err) {
						if (err) {
							if (next) {
								next(err);
							} else {
...
function check_struct(e) {
	...
	for (var k in e.data) {
		...
		if (e.props[k].primary) {
			...
			if (e.changes.indexOf(k) == -1) continue;
